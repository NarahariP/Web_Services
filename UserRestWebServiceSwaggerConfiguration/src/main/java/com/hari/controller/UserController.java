...
public class UserController {
	...
	public User getUserById(@PathVariable int id) {
		 ...
		 */
		/*
		 * return userRepository.findById(id).map(user -> { return new
		 * EntityModel<>(user).add(linkTo(methodOn(this.getClass()).getAllUsers()).
		 * withRel("all-users")); }).orElseThrow(() -> new UserNotFoundException("Id - "
		 * + id));
		 */
